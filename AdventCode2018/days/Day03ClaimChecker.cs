...
namespace day03
{
    public class Day03ClaimChecker
    {
        /*
        ...
        public List<Claim> ParseData(string datapath)
        {
            ...
            {
                claims.Add(Claim.MakeClaim(row));
            }

            return claims;
        }

        public void MarkupCloth(int[,] cloth, Claim claim)
        {
        ...
        public void MarkupCloth(int[,] cloth, List<Claim> claims)
        {
            foreach (var claim in claims)
            {
                MarkupCloth(cloth, claim);
            }
        }

        public int OverlappingInches(int[,] cloth, int dimX, int dimY)
        {
            int overlap = 0;
        ...
        public bool DoesClaimOverlap(int[,] cloth, Claim claim)
        {
            for (int i = claim.X; i < claim.X + claim.Width; i++)
            {
                for (int j = claim.Y; j < claim.Y + claim.Height; j++)
